...
export default function Contact() {
    ...
    const SendButton = () => {
        if (!formInputs.email || !formInputs.message) {
            return <Button disabled={true} className="col-span-2 dark:hover:bg-stone-800 hover:bg-zinc-50 dark:text-white text-black" variant="contained" endIcon={< RuleIcon />}>Complete form</ Button>
        }
        else {
            ...
            <Container>
                <div className="flex flex-col justify-between items-center space-y-6">
                    ...
                    <Box className="grid grid-cols-2 gap-4 p-4 w-full h-auto bg-white border rounded-md border-zinc-200 dark:bg-black dark:border-zinc-800 shadow-md dark:shadow-zinc-950">
                        ...
                        <TextField disabled={!session} helperText={error ? 'Email must contain an @ symbol' : ''} error={error} required key={"email"} onChange={(e) => handleChange(e)} value={formInputs.email} className='dark:bg-stone-950' id="outlined-basic" label="E-mail" name="email" variant="outlined" InputProps={{ placeholder: "", style: placeholderStyle }} InputLabelProps={{ style: placeholderStyle }} />
