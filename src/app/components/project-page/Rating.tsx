            ...
            <>
                <Tooltip className="cursor-pointer" title={session && session.user ? "Leave a rating" : "Sign in to leave a rating"}>
                    <div className='flex flex-row'>
                        <Rating name="half-rating" defaultValue={(rating / numberOfVotes) ?? 0} precision={0.5} onChange={(e, i) => createRating(i ?? 3)} disabled={(!session)} />
                    </div>
                </Tooltip>
                ...
                <Tooltip className="cursor-pointer" title={session && session.user ? "Leave a rating" : "Sign in to leave a rating"}>
                    <div className='flex flex-row'>
                        <Link href="/api/auth/signin">
                            <Rating name="half-rating" defaultValue={(rating / numberOfVotes) ?? 0} precision={0.5} onClick={() => signIn()} disabled={true} />
                        </Link>
                    </div>
                </Tooltip>
                <h1>{`(${numberOfVotes ?? 0}) votes`}</h1>
